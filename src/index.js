...
SmoothScrollr.prototype = (function () {
      ...
      if (this.move.prev !== this.move.dest) {
        ...
        for (let i = this.sections.length - 1; i >= 0; i--) {
          ...
          if (this.move.current > this.sections[i].offset && this.move.current < this.sections[i].limit) {
            this.sections[i].el.style.transform =
              this.events.enableSmoothScroll &&
              !this.prevent &&
              `translate3D(${this.config.direction === "horizontal" ? this.move.position : 0}px,${this.config.direction === "vertical" ? this.move.position : 0}px, 0)`;
  ...
  const init = function init(config) {
      ...
      //bind events
      const eventOpt = (({ touch, listened, parallax, speed, multFirefox, touchSpeed, jump, section }) => ({ touch, listened, parallax, speed, multFirefox, touchSpeed, jump, section }))(this.config);
    ...
    on = function on(event, cb, context = this.DOM.scroller) {
      const events = ["scroll", "collisionTop", "collisionBottom", "collisionEnded", "resize"];
